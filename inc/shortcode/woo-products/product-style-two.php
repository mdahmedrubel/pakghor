...
class WPBakeryShortcode_pakghor_product_two extends WPBakeryShortcode {
	protected function content( $atts, $content = null ) {
		...
		pakghor_product_two( $section_top_icon, $section_top_image,  $section_top_image_width, $title, $subtitle, $item_count, $single_page_link, $show_rating, $pakghor_woo_excerpt_limit, $category_options, $category, $tag, $postid, $is_product_menu, $offset, $order, $orderby, $section_bg_image_left, $lbg_size, $lbg_position, $section_bg_image_right, $rbg_size,  $rbg_position, $fc_class );
		return ob_get_clean();
	}
}

// Taxonomy query for this post type
if( ! function_exists('pakghor_product_two_taxonomy') ) {
    function pakghor_product_two_taxonomy() {
...
if( ! function_exists('pakghor_product_two_category') ) {
    function pakghor_product_two_category($taxonomy){
        global $wpdb;
...
$terms = pakghor_product_two_category('product_cat');
$data = array(
...
vc_map( array(
	"name"			=> esc_html__( "Pakghor Product Style Two", "pakghor" ),
	"base"			=> "pakghor_product_two",
	"class"			=> "",
    "icon"          => "fa fa-cutlery",
...
function pakghor_product_two( $section_top_icon, $section_top_image,  $section_top_image_width, $title, $subtitle, $item_count, $single_page_link, $show_rating, $pakghor_woo_excerpt_limit, $category_options, $category, $tag, $postid, $is_product_menu, $offset, $order, $orderby, $section_bg_image_left, $lbg_size, $lbg_position, $section_bg_image_right, $rbg_size,  $rbg_position, $fc_class ) {
    ...
    $product_wrapper_class = array( 'food-menu-item-wrapper' );
    $top_img_class = array();
    ...
    // Section class
    $sec_classes = array('food-menu', 'section-padding');
    if( !empty($fc_class) ) {
    ...
    if( !empty($section_bg_image_left) || !empty($section_bg_image_right) ){
        $section_attr[] ='style="background: 
        url( ' .esc_url( $section_bg_image_left['0'] ). ' ) no-repeat, url('.esc_url($section_bg_image_right['0']).') no-repeat;
...
?>
	...
	<section <?php echo implode(' ', $section_attr) ?>>
		<div class="container">
            ...
            <?php   
                    if( $category_options ) :
                        $fproduct_style_two_query = new WP_Query( array(
                                'post_type'             => 'product',
                    ...
                    if ( $fproduct_style_two_query->have_posts() ) :
                      
                        /* Start the Loop */
                        while ( $fproduct_style_two_query->have_posts() ) : $fproduct_style_two_query->the_post(); ?>
