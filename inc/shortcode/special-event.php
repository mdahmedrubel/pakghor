...
class WPBakeryShortcode_pakghor_event extends WPBakeryShortcode {
	protected function content( $atts, $content = null ) {
		...
		ob_start();
			...
			pakghor_event( $section_top_icon, $section_top_image,  $section_top_image_width, $title, $subtitle, $item_count, $event_excerpt_word, $order, $orderby, $category_options, $category, $tag, $postid, $offset, $section_bg_image,$bg_size, $bg_repeat, $bg_position, $section_overlay, $fc_class );
		return ob_get_clean();
	}
}

// Taxonomy query for this post type
if( ! function_exists('pakghor_event_taxonomy') ) {
    function pakghor_event_taxonomy() {
        $categories = get_terms(
                array(
                    'taxonmy'       =>'event_category', 
                    'hide_empty'    => true
                )
            );
        if($categories){
...
if( ! function_exists('pakghor_event_category') ) {
    function pakghor_event_category($taxonomy){
        global $wpdb;
...
$terms = pakghor_event_category('event_category');
$data = array(
...
vc_map( array(
	"name"			=> esc_html__( "Pakghor Event", "pakghor" ),
	"base"			=> "pakghor_event",
	...
	"params"		=> array(
        array(
            "type"			=> "iconpicker",
            "heading"		=> esc_html__( "Event Section Top Icon", "pakghor" ),
            "param_name"	=> "section_top_icon",
            "group"			=> esc_html__( "General", "pakghor" ),
        ),
        array(
            "type"			=> "attach_image",
            "heading"		=> esc_html__( "Event Section Top Image", "pakghor" ),
        ...
        array(
            "type"			=> "textfield",
            "heading"		=> esc_html__( "Event Section Title", "pakghor" ),
            "param_name"	=> "title",
            "admin_label"	=> true,
            "group"			=> esc_html__( "General", "pakghor" ),
        ),
        ...
        array(
            "type"			=> "textfield",
            "heading"		=> esc_html__( "Event Count", "pakghor" ),
            "param_name"	=> "item_count",
            "group"			=> esc_html__( "General", "pakghor" ),
            "description"   => esc_html__( "How many Testimonials want to display? Default is 3", "pakghor" ),
            "std"			=> 3,
        ),
        array(
            "type"			=> "textfield",
            "heading"		=> esc_html__( "Event Excerpt Word Limit", "pakghor" ),
            "param_name"	=> "event_excerpt_word",
            "admin_label"	=> true,
            "std"			=> 45,
            "group"			=> esc_html__( "General", "pakghor" ),
            "description"   => esc_html__( "The limit of Event Excerpt word limit ,Default is 15", "pakghor" ),
        ),
        array(
            "type"          => "dropdown",
            "heading"       => esc_html__( "Select Event Category", "pakghor" ),
        ...
        array(
            ...
            "admin_label"	=> true,
            "value"     	=> array(
                esc_html__( "Select Event Order","pakghor")   => "",
        ...
        array(
            "type"          => "dropdown",
            "heading"       => esc_html__( "Event Order By", "pakghor" ),
            "group" 		=> esc_html__("Post Query", "pakghor"),
            "param_name"    => "orderby",
            "value"     	=> array(
                esc_html__( "Select Event Order by","pakghor") => "",
...
function pakghor_event( $section_top_icon, $section_top_image,  $section_top_image_width, $title, $subtitle, $item_count, $event_excerpt_word, $order, $orderby,$category_options, $category, $tag, $postid, $offset, $section_bg_image,$bg_size, $bg_repeat, $bg_position, $section_overlay, $fc_class ) { 
    ...
    $wrapper_class = array('swiper-wrapper') ;
	...
	// Css Classes
    $sec_classes = array('special-event');
    if( !empty($fc_class) ) {
				...
				<div class="row">
					...
					<div class="swiper-container">
				        <div <?php echo wp_kses_post($wrapper_attr); ?>>
					        <?php 
                                if( $category_options ) :
                                    $pakghor_events = new WP_Query( array(
								...
								if( $pakghor_events -> have_posts() ) :
									while ( $pakghor_events -> have_posts() ) : $pakghor_events -> the_post();

								$_pakghor_event_page_options = $pakghor_event_time = $pakghor_event_location		= '';
								if( function_exists('cs_get_option') ){
				            ...
				            <div class="swiper-slide">
				            	<div class="special-event-item">
				            		...
				            		<div class="special-event-content">
				            			...
				            			<?php if( !empty( $pakghor_event_location ) || !empty($pakghor_event_time) ) :?>
				            			<div class="meta-post">
				            				<ul>
				            					<?php if(! empty( $pakghor_event_time) ) : ?>
				            					<li>
				            						<i class="fa fa-calendar"></i><?php echo esc_attr($pakghor_event_time); ?></li>
				            					<?php endif; ?>
				            					<?php if(! empty($pakghor_event_location) ) : ?>
				            					<li>
				            						<i class="fa fa-map-marker"></i><?php echo esc_attr($pakghor_event_location); ?></li>
				            					<?php endif; ?>
				            				</ul>
				            			</div><!-- meta post -->
				            			<?php endif; ?>
							        	<?php if(!empty($event_excerpt_word)): ?>
							        		<p><?php echo wp_trim_words(get_the_content(), $event_excerpt_word,''); ?></p>
							        	<?php endif; ?>
                                        <div class="event-btn">
                                            <a href="<?php the_permalink(); ?>" class="button"><?php echo esc_html__('Event Details','pakghor') ?></a>
                                        </div>
				            		</div><!-- special-event-content -->
				            	</div><!-- special-event-item -->
				            </div><!-- swiper-slide -->
